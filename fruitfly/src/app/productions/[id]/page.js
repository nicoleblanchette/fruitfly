...
const ProductionGallery = ({ production }) => {
  return (<ul className="columns-1 gap-5 sm:columns-2 sm:gap-8 md:columns-3 lg:columns-4 [&>img:not(:first-child)]:mt-8">
    {production.gallery.map((image, i) => {
      return (<li className="my-8"><Image src={image.src} alt={image.alt} key={i} /></li>)
    })}
  </ul>)
} 
